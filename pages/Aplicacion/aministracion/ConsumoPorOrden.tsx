...
const ConsumoPorOrden = ({handleCancel, send, handleInitial}: {handleCancel: () => void, handleInitial: () => void, send: (action: { type: string }) => void,}) => {
  ...
  const handleOrdenSubmit = async () => {
    try {
      ...
      if (data) {
        // Transformación de los datos para renombrar la columna 'total_quantity' a 'cantidad'
        const transformedData = data.map((item) => ({
          ...item,
          cantidad: item.total_quantity, // Renombrar 'total_quantity' a 'cantidad'
        }));
  
        setConsumo(transformedData); // Asignar los datos transformados
        setError("");
      } else {
