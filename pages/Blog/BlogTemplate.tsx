  ...
  return (
    <Layout>
      <article className="max-w-4xl mx-auto bg-whiteser shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="relative h-[34rem] bg-darkser"> {/* Aumenta la altura aquí */}
          ...
          {content ? (
              content.split('\n\n').map((paragraph, index) => {
                if (paragraph.startsWith('## ')) {
                  ...
                  return <h4 key={index} className="text-lg font-semibold mt-4 mb-2 text-ser">{paragraph.slice(5)}</h4>;
                } else if (paragraph.startsWith('##### ')) {
                  return <h5 key={index} className="text-base font-semibold mt-4 mb-2 text-ser">{paragraph.slice(6)}</h5>;
                } else {
