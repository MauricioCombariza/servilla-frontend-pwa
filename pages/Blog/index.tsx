    ...
    <Layout>
      <div className="min-h-screen bg-lightser p-8">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {blogs.map((blog) => (
            <Link key={blog.slug} href={`/blog/${blog.slug}`} className="block p-6 bg-white shadow-md rounded-lg hover:bg-ser transition-all duration-200">
              <h2 className="text-2xl font-semibold text-darkser mb-2">{blog.title}</h2>
              <p className="text-sm text-gray-500">
                {format(parse(blog.date, 'd/M/yyyy', new Date()), 'dd/MM/yyyy', { locale: es })}
              </p>
              <p className="text-sm text-ser">{blog.subtitle}</p>
            </Link>
          ))}
        </div>
      </div>
